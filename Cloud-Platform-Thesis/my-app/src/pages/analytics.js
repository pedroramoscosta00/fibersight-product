...
// GeoJSON data import
const geojsonDataPromise = import('../fiber-points/fiber_chart_data2.geojson')
  .then(module => {
...
const generateColorMap = (features) => {
  ...
  features.forEach((feature, index) => {
    ...
    const hue = (index * (360 / features.length)) % 360;
    colorMap[id] = `hsl(${hue}, 70%, 50%)`;

    //Add color directly to feature properties
    if (feature.properties) {
      feature.properties.color = colorMap[id];
    }
  });
  return colorMap;
};
...
const TimeSeriesChart = ({ processedFeatures, colorMap, activeParameter }) => {
  ...
  const plotData = processedFeatures.map((feature) => {
    ...
    const isGroup = id.includes('Group'); // Alternative check
  ...
  useEffect(() => {
    const loadAndProcessData = async () => {
      try {
        ...
        const module = await import('../fiber-points/fiber_chart_data2.geojson');
